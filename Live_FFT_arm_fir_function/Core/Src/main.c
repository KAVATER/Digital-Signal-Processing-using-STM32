...
/* ===== ARM fir_function ===== */
#define NUM_TAPS   fir_len
#define BLOCK_SIZE live_ecg_len
...
volatile float32_t swv_fir_sample;
/* USER CODE END 0 */
...
//	 	          }
	 	        ...
	 	        for (uint32_t i = 0; i < BLOCK_SIZE; i++)
	 	        	...
	 	        	{
	 	        	block_in[i] = (float32_t)adc_buff[i];
	 	        	}
...
//}
	 	       ...
	 	       for (uint32_t i = 0; i < BLOCK_SIZE; i++)
	 	            ...
	 	            // plot_signal((float32_t*)adc_buff,adc_buff_len);
		           // plot_signal_2((float32_t*)adc_buff, adc_buff_len,block_out ,BLOCK_SIZE );
